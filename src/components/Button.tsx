...
function Button({ children, type, width, height, bgColor, purpose, className, disabled, onClick }: ButtonProps) {
  ...
  if (purpose == "count") {
    return (
      <div className={`flex justify-center items-center border border-solid border-gray-200`} style={{width: `${width}rem`}}>
  ...
  if (purpose == "nav") {
    return (
      <button
        className={`${className} flex justify-center items-center w-56 h-56 border border-solid border-gray-100 rounded hover:border-primary focus:border-primary active:border-primary`}
        onClick={onClick}
      >
        { children }
      </button>
  ...
  return (
    <button type={type} className={`${className} ${baseButtonClassName} label text-md w-full ${AddButtonClassName} rounded`} style={{width: `${width}rem`, height: `${height}rem`}} onClick={onClick} disabled={disabled}>{ children }</button>
  )
}
